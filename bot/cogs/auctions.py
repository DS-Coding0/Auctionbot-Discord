...
class AuctionsCog(commands.Cog):
    ...
    async def list_auctions_prefix(self, ctx: commands.Context):
        ...
        lines = [f'#{auction.id} | item={auction.item_id} | {auction.status} | {auction.current_price}' for auction in auctions[:10]]
        await ctx.reply('\n'.join(lines))

    @app_commands.command(name='auctions', description='List auctions')
    async def list_auctions_slash(self, interaction):
        auctions = self.service.list_auctions()
        if not auctions:
    ...
    async def create_auction(self, interaction, show_id: int, item_id: int):
        auction = self.service.create_auction(show_id=show_id, item_id=item_id, status='draft')
        await interaction.response.send_message(f'Created auction draft #{auction.id} for item {item_id}.', ephemeral=True)

    @app_commands.command(name='start_auction', description='Start an auction by ID and post live buttons')
    @app_commands.describe(auction_id='Auction ID')
    async def start_auction(self, interaction, auction_id: int):
    ...
    async def search_auction(self, interaction):
        view = AuctionSearchView(self.service)
        await interaction.response.send_message('Search an item to start its auction.', view=view, ephemeral=True)


async def setup(bot):
    cog = AuctionsCog(bot)
    await bot.add_cog(cog)
    bot.tree.add_command(cog.list_auctions_slash)
    bot.tree.add_command(cog.create_auction)
    bot.tree.add_command(cog.start_auction)
    bot.tree.add_command(cog.search_auction)