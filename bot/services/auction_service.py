...
class AuctionService:
    ...
    def create_auction(self, show_id: int, item_id: int, current_price: float = 0, status: str = 'scheduled', channel_id=None, message_id=None, ends_at=None):
    ...
    def bid(self, auction_id: int, buyer_id: int, amount: float, min_increment: float | None = None, seller_id: int | None = None):
        ...
        increment = float(min_increment if min_increment is not None else getattr(item, 'min_increment', config.min_bid_increment))
    ...
    def instant_buy(self, auction_id: int, buyer_id: int):
        ...
        auction = self.repo.update_price(auction_id, float(item.instant_buy_price), status='ended')
